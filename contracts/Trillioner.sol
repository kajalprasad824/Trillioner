...
contract Trillioner is Context, IERC20, Ownable, Pausable, ReentrancyGuard {
     ...
     */
    function mint(uint256 value) public onlyOwner whenNotPaused{
    ...
    function _transfer(
        ...
        if (locks[sender].lockedToken > 0) {
            uint256 withdrawable = balanceOf(sender) -
                locks[sender].remainingLockedToken;
            require(
                amount <= withdrawable,
    ...
    function unlock(address target_) external {
        ...
        uint256 receivableToken = (lockedToken *
            (remainingMonth * openingPercentage)) / 100;
